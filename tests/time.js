...
// ---------- ② 讀不到存摺就不寫 ----------
{
  ...
  ok(chips(t).length === 11, `內建十一項事項, 實得 ${chips(t).length}`);
...
// ---------- ③ 打卡：點下一件上一件就結束；停止；同一分鐘的零長度段拿掉 ----------
{
  ...
  t.click(chip(t, "hw"));                                   // 同一分鐘：吃飯是 0 分鐘，拿掉
  ...
  f.querySelector("[name=s]").value = "17:15"; f.querySelector("[name=e]").value = "18:40"; f.dispatchEvent(new t.w.Event("submit", {bubbles:true, cancelable:true}));
  ok(tl(t)[1].s === 1035 && tl(t)[1].e === 1120, `改得了, 實得 ${JSON.stringify(tl(t)[1])}`);
  ...
  ok(/寫功課/.test(lg) && /1 小時 25 分/.test(lg) && /練琴/.test(lg), `圖例要有本週合計, 實得「${lg}」`);
...
// ---------- ⑤ 事項清單：家長改的、壞資料、只留 90 天 ----------
{
  ...
  ok(chips(t2).length === 11, "清單全壞就退回內建十一項");
  // 90 天
...
// ---------- ⑥ 單字闖關：入口、設定頁的事項編輯（要密碼）、內建清單同一份 ----------
{
  ...
  ok(a.ev("tlActs().length") === 12 && a.ev("tlActs()[11].name") === "游泳" && a.ev("tlActs()[11].em") === "🏊" && !a.ev("tlActs()[11].p") && /^c/.test(a.ev("tlActs()[11].id")), `加得了（confirm 說是一段時間）, 實得 ${JSON.stringify(a.ev("tlActs()"))}`);
  ok(a.ev("tlActs()[0].id") === "home" && a.ev("tlActs()[1].id") === "wc", "內建十一項要一起寫進去、順序不變（顏色照位置配）");
  a.w.confirm = () => false;                                  // 「取消」= 時間點
  a.say("1234", "出門", "🚌"); a.click(a.$("btnTlActs"));
  ok(a.ev("tlActs().length") === 13 && a.ev("tlActs()[12].p") === 1, "新增時可以選成時間點");
  a.w.confirm = () => true;
  a.say("1234", "游泳", ""); a.click(a.$("btnTlActs"));
  ok(a.ev("tlActs().length") === 13, "同名不得重複加");
  a.say("1234", "a|b;c 很長很長很長很長的名字", ""); a.click(a.$("btnTlActs"));
  ok(a.ev("tlActs()[13].name") === "a b c 很長", `名稱要去分隔符、限 8 字, 實得「${a.ev("tlActs()[13].name")}」`);
  a.say("1234", "14"); a.click(a.$("btnTlActs"));            // 沒用過 → 刪
  ok(a.ev("tlActs().length") === 13, "沒用過的事項輸入編號可以刪掉");
  a.ev(`SHARED.tl = {[dayKey()]:[{a:"hw", s:1000, e:1060}]}; saveShared();`);
  a.say("1234", "4"); a.click(a.$("btnTlActs"));             // 用過 → 關
  ok(a.ev("tlActs().length") === 13 && a.ev("tlActs()[3].off") === true, "用過的事項只能關掉，不得刪");
  a.say("1234", "4"); a.click(a.$("btnTlActs"));
  ok(!a.ev("tlActs()[3].off"), "再輸入一次就打開");
  for(let i = 0; i < 5; i++){ a.say("1234", "新" + i, ""); a.click(a.$("btnTlActs")); }
  ok(a.ev("tlActs().length") === 16, `最多 16 項, 實得 ${a.ev("tlActs().length")}`);
  // time.html 要吃到
  const t = boot(timeHtml, "time.html", ls=>ls.setItem("cq-shared-v1", a.ls.getItem("cq-shared-v1")), {min:1000});
  ok(chips(t).length === 16 && /游泳/.test(t.$("actGrid").textContent) && chip(t, a.ev("tlActs()[12].id")).classList.contains("pt"), "時間紀錄那頁要看到家長改的清單，含時間點");
}

// ---------- ⑦ 備份碼往返、雲端摘要、回到單字闖關時補傳 ----------
{
  const tlData = {[y2]:[{a:"hw", s:1000, e:1060, x:1}], [y1]:[{a:"home", s:1015, p:1}, {a:"wc", s:1022, e:1030}, {a:"eat", s:1030, e:1100}, {a:"sleep", s:1300, p:1}], [today]:[{a:"hw", s:1000, e:1050}, {a:"play", s:1060}]};
  ...
  ok(sum.tl.days[today][1].e === undefined && sum.tl.days[y2][0].x === 1 && sum.tl.days[y1][0].p === 1, "摘要要保留進行中、忘了停、時間點的標記");
...
// ---------- ⑧ 家長頁：有打卡才有「時間」分頁；色帶、圖例；壞資料 ----------
{
  ...
  p = bootP(mk({tl:{acts:[{id:"home", em:"🏠", name:"到家", p:1}, {id:"hw", em:"📝", name:"寫功課"}, {id:"play", em:"🎮", name:"玩"}], days:{[y2]:[{a:"hw", s:1000, e:1030}], [y1]:[{a:"home", s:1000, p:1}, {a:"hw", s:1020, e:1080, x:1}, {a:"play", s:1090, e:1140}], [D]:[{a:"hw", s:1000}]}}})); await wait();
  ...
  ok(sec.querySelectorAll(".band .row i").length === 5 && sec.querySelectorAll(".band .row i.trunc").length === 2 && sec.querySelectorAll(".band .row i.pt").length === 1, `五筆：忘了停與進行中的兩段是斜線、到家是細線, 實得 ${sec.querySelectorAll(".band .row i").length}/${sec.querySelectorAll(".band .row i.trunc").length}/${sec.querySelectorAll(".band .row i.pt").length}`);
  ...
  p = bootP(mk({tl:{acts:[{id:BOOM, em:BOOM, name:BOOM, p:BOOM}, BOOM, null], days:{[D]:[{a:BOOM, s:BOOM, e:BOOM, x:BOOM, p:BOOM}, BOOM, {a:"hw", s:-5, e:99999}, {a:"hw", s:BOOM, p:1}], [BOOM]:"x"}}})); await wait();
